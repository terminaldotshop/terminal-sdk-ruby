...
Most of the SDK is generated code. Modifications to code will be persisted between generations, but may result in merge conflicts between manual patches and changes from the generator. The generator will never modify the contents of `lib/terminal_shop/helpers/` and `examples/` directory.
...
```ruby
gem "terminal-shop", path: "<path-to-repo>"
```
