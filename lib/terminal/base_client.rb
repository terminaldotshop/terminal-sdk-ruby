...
module Terminal
  ...
  class BaseClient
    ...
    # @private
    #
    # @return [Terminal::PooledNetRequester]
    ...
    def initialize(
      ...
      @requester = Terminal::PooledNetRequester.new
      @headers = Terminal::Util.normalized_headers(
        {
          "X-Stainless-Lang" => "ruby",
          "X-Stainless-Package-Version" => Terminal::VERSION,
      ...
      @base_url = Terminal::Util.parse_uri(base_url)
    ...
    #
    #   @option req [Terminal::Converter, Class, nil] :model
    #
    ...
    private def build_request(req, opts)
      ...
      path = Terminal::Util.interpolate_path(uninterpolated_path)
      ...
      body =
        ...
        in :get | :head | :options | :trace
          ...
          Terminal::Util.deep_merge(*[req[:body], opts[:extra_body]].compact)
        end

      url = Terminal::Util.join_parsed_uri(@base_url, {**req, path: path})
      headers, encoded = Terminal::Util.encode_content(headers, body)
    ...
    private def should_retry?(status, headers:)
      coerced = Terminal::Util.coerce_boolean(headers["x-should-retry"])
      case [coerced, status]
      in [true | false, _]
        coerced
    ...
    private def retry_delay(headers, retry_count:)
      ...
      span = retry_header && Terminal::Util.suppress(ArgumentError) do
    ...
    private def follow_redirect(request, status:, location_header:)
      ...
      location =
        Terminal::Util.suppress(ArgumentError) do
          URI.join(url, location_header)
        end

      unless location
        message = "Server responded with status #{status} but no valid location header."
        raise Terminal::APIConnectionError.new(url: url, message: message)
      end
      ...
      # from undici
      if Terminal::Util.uri_origin(url) != Terminal::Util.uri_origin(location)
    ...
    private def send_request(request, redirect_count:, retry_count:, send_retry_header:)
      ...
      rescue Terminal::APIConnectionError => e
        status = e
      end
      ...
      in Terminal::APIConnectionError if retry_count >= max_retries
        raise status
      in (400..) if no_retry || (response && !should_retry?(status, headers: response))
        body = Terminal::Util.decode_content(response, suppress_error: true)

        raise Terminal::APIStatusError.for(
          url: url,
          status: status,
          body: body,
          request: nil,
      ...
      in (400..) | Terminal::APIConnectionError
    ...
    #   @option req [Terminal::Converter, Class, nil] :model
    #
    #   @option req [Terminal::RequestOptions, Hash{Symbol=>Object}, nil] :options
    #
    # @param response [nil]
    #
    # @return [Object]
    ...
    private def parse_response(req, response)
      parsed = Terminal::Util.decode_content(response)
      unwrapped = Terminal::Util.dig(parsed, req[:unwrap])

      page = req[:page]
      model = req.fetch(:model, Terminal::Unknown)
      case [page, model]
      in [Class, Class | Terminal::Converter | nil]
        page.new(client: self, req: req, headers: response, unwrapped: unwrapped)
      in [nil, Class | Terminal::Converter]
        Terminal::Converter.coerce(model, unwrapped)
      in [nil, nil]
        unwrapped
      end
    end
    ...
    def request(req)
      ...
      Terminal::RequestOptions.validate!(opts)
    ...
    #
    def inspect
      base_url = Terminal::Util.unparse_uri(@base_url)
