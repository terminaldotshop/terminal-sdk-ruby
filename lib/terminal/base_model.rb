...
module Terminal
  ...
  module Converter
    ...
    class << self
      # @private
      #
      # @param spec [Hash{Symbol=>Object}, Proc, Terminal::Converter, Class] .
      #
      ...
      def type_info(spec)
        ...
        in Proc
          spec
        in Terminal::Converter | Class
          -> { spec }
        in true | false
          -> { Terminal::BooleanModel }
      ...
      #
      # @param target [Terminal::Converter, Class]
      # @param value [Object]
      #
      # @return [Object]
      #
      ...
      def try_strict_coerce(target, value)
        ...
        in Class
          ...
          in [-> { _1 <= Date || _1 <= Time }, String]
            Terminal::Util.suppress(ArgumentError, Date::Error) do
  ...
  class Unknown
    ...
    # @return [Boolean]
    #
    def self.==(other) = other.is_a?(Class) && other <= Terminal::Unknown
  ...
  class BooleanModel
    ...
    # @return [Boolean]
    #
    def self.==(other) = other.is_a?(Class) && other <= Terminal::BooleanModel
  ...
  class Enum
    ...
    #
    def self.==(other)
      other.is_a?(Class) && other <= Terminal::Enum && other.values.to_set == values.to_set
    end
  ...
  class Union
    ...
    # @private
    #
    # @param key [Symbol, Hash{Symbol=>Object}, Proc, Terminal::Converter, Class]
    #
    # @param spec [Hash{Symbol=>Object}, Proc, Terminal::Converter, Class] .
    #
    #   @option spec [NilClass, TrueClass, FalseClass, Integer, Float, Symbol] :const
    ...
    private_class_method def self.variant(key, spec = nil)
      variant_info =
        case key
        in Symbol
          [key, Terminal::Converter.type_info(spec)]
        in Proc | Terminal::Converter | Class | Hash
          [nil, Terminal::Converter.type_info(key)]
        end
    ...
    # @param value [Object]
    #
    # @return [Terminal::Converter, Class, nil]
    #
    private_class_method def self.resolve_variant(value)
      case [@discriminator, value]
      in [_, Terminal::BaseModel]
        value.class
      in [Symbol, Hash]
        key =
        ...
        resolved.nil? ? Terminal::Unknown : resolved.call
      else
        nil
      end
    end
    ...
    #
    def self.==(other)
      other.is_a?(Class) && other <= Terminal::Union && other.variants == variants
    end
    ...
    def self.coerce(value)
      if (variant = resolve_variant(value))
        return Terminal::Converter.coerce(variant, value)
      end
      ...
      known_variants.each do |_, variant_fn|
        ...
        case Terminal::Converter.try_strict_coerce(variant, value)
        in [true, coerced, _]
          return coerced
        in [false, true, score]
      ...
      variant.nil? ? value : Terminal::Converter.coerce(variant, value)
    end
    ...
    def self.dump(value)
      if (variant = resolve_variant(value))
        return Terminal::Converter.dump(variant, value)
      end
  ...
  class ArrayOf
    ...
    # @return [Boolean]
    #
    def ==(other) = other.is_a?(Terminal::ArrayOf) && other.item_type == item_type
    ...
    def coerce(value)
      ...
      in Enumerable unless value.is_a?(Hash)
        value.map { |item| Terminal::Converter.coerce(type, item) }
      else
        value
      end
    end
    ...
    def dump(value)
      ...
      case value
      in Enumerable unless value.is_a?(Hash)
        value.map { |item| Terminal::Converter.dump(type, item) }.to_a
      else
        value
      end
    end
    ...
    def try_strict_coerce(value)
      ...
      in Array
        ...
        mapped =
          value.map do |item|
            case Terminal::Converter.try_strict_coerce(type, item)
            in [true, coerced, score]
              tally += score
              coerced
            in [false, true, score]
    ...
    # @private
    #
    # @return [Terminal::Converter, Class]
    #
    protected def item_type = @item_type_fn.call

    # @private
    #
    # @param type_info [Hash{Symbol=>Object}, Proc, Terminal::Converter, Class]
    #
    ...
    def initialize(type_info, spec = {})
      @item_type_fn = Terminal::Converter.type_info(type_info || spec)
    end
  end
  ...
  class HashOf
    ...
    # @return [Boolean]
    #
    def ==(other) = other.is_a?(Terminal::HashOf) && other.item_type == item_type
    ...
    def coerce(value)
      ...
      in Hash
        value.to_h do |key, val|
          coerced = Terminal::Converter.coerce(type, val)
    ...
    def dump(value)
      ...
      in Hash
        value.transform_values do |val|
          Terminal::Converter.dump(type, val)
        end
      else
        value
      end
    ...
    def try_strict_coerce(value)
      ...
      in Hash
        ...
        mapped =
          value.transform_values do |val|
            case Terminal::Converter.try_strict_coerce(type, val)
            in [true, coerced, score]
              tally += score
              coerced
            in [false, true, score]
  ...
  class BaseModel
    ...
    def self.known_fields
      @known_fields ||= (self < Terminal::BaseModel ? superclass.known_fields.dup : {})
    end
    ...
    private_class_method def self.add_field(name_sym, required:, type_info:, spec:)
      type_fn, info =
        case type_info
        in Proc | Class | Terminal::Converter
          [Terminal::Converter.type_info({**spec, union: type_info}), spec]
        in Hash
          [Terminal::Converter.type_info(type_info), type_info]
        end
      ...
      define_method(name_sym) do
        ...
        Terminal::Converter.coerce(field_type, value)
      rescue StandardError
        name = self.class.name.split("::").last
        raise Terminal::ConversionError.new(
    ...
    # @private
    #
    # @param value [Terminal::BaseModel, Hash{Object=>Object}, Object]
    #
    # @return [Terminal::BaseModel, Object]
    #
    def self.coerce(value)
      case (coerced = Terminal::Util.coerce_hash(value))
      in Hash
        new(coerced)
      else
        value
    ...
    # @private
    #
    # @param value [Terminal::BaseModel, Object]
    #
    # @return [Hash{Object=>Object}, Object]
    #
    def self.dump(value)
      unless (coerced = Terminal::Util.coerce_hash(value)).is_a?(Hash)
        return value
      end
      ...
      values = coerced.filter_map do |key, val|
        ...
        in nil
          ...
          in :coerce
            ...
            target = type_fn.call
            [api_name, Terminal::Converter.dump(target, val)]
          end
        end
      end.to_h
    ...
    def self.try_strict_coerce(value)
      ...
      known_fields.each_value do |field|
        ...
        in [_, true]
          ...
          case Terminal::Converter.try_strict_coerce(target, item)
          in [true, coerced, score]
            tally += score
    ...
    #
    # @param data [Hash{Symbol=>Object}, Terminal::BaseModel]
    #
    def initialize(data = {})
      case (coerced = Terminal::Util.coerce_hash(data))
      in Hash
        @data = coerced.transform_keys(&:to_sym)
      else
        raise ArgumentError.new("Expected a #{Hash} or #{Terminal::BaseModel}, got #{data.inspect}")
      end
    end
